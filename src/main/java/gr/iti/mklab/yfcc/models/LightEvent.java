...
public class LightEvent {
    ...
    public double getMinRadious() {
    	...
    	if(medianGeo != null){
    		...
    		for(Point p : points) {	
    			Double distance = GeodesicDistanceCalculator.vincentyDistance(p.getLatitude(), p.getLongitude(), 
    ...
    public long getMeanTakenTime() {
    	meanTakenTime = timeline.getMean();
    	
    	return meanTakenTime;
    }
    
    public long getMedianTakenTime() {
    	medianTakenTime = meanTakenTime = timeline.getMedian();
    ...
    public void merge(LightEvent event) {
    	merges++;
    	
    	mergedEvents.add(event.getId());
    	mergedEvents.addAll(event.mergedEvents);
    	
    	this.items.addAll(event.items);
    	this.timeline.merge(event.timeline);
    	this.users.addAll(event.users);
    	
    	this.machineTags.addAll(event.machineTags);
    	
    	this.points.addAll(event.points);
    	
    	this.titleVector.mergeVector(event.titleVector);
    	this.descriptionVector.mergeVector(event.descriptionVector);
    	this.tagsVector.mergeVector(event.tagsVector);
    	
    	for(String tag : event.tags.keySet()) {
    		if(this.tags.containsKey(tag)) {
    			this.tags.put(tag, this.tags.get(tag) + event.tags.get(tag));
    		}
    		else {
    			this.tags.put(tag, event.tags.get(tag));
    		}
    	}
    	
    	for(String person : event.persons.keySet()) {
    		if(this.persons.containsKey(person)) {
    			this.persons.put(person, this.persons.get(person) + event.persons.get(person));
    		}
    		else {
    			this.persons.put(person, event.persons.get(person));
    		}
    	}
    	
    	for(String organization : event.organizations.keySet()) {
    		if(this.organizations.containsKey(organization)) {
    			this.organizations.put(organization, this.organizations.get(organization) + event.organizations.get(organization));
    		}
    		else {
    			this.organizations.put(organization, event.organizations.get(organization));
    		}
    	}
    	
    	for(String location : event.locations.keySet()) {
    		if(this.locations.containsKey(location)) {
    			this.locations.put(location, this.locations.get(location) + event.locations.get(location));
    		}
    		else {
    			this.locations.put(location, event.locations.get(location));
    		}
    	}
	...
	public boolean isSameEvent(LightEvent e) {
	...
	public double userSetSimilarity(LightEvent event2) {
	...
	public double tagsOverlap(LightEvent event2) {
	...
	public double overlap(LightEvent other) {
	    Set<String> intersection = new HashSet<String>();
	...
	public DBObject toDBObject() {
		...
		List<String> itemIds = new ArrayList<String>(items);
